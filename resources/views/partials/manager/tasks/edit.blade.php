    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form role="form" method="post" action="{{route('manager.tasks.update',$model->id)}}">
                    @csrf
                    @method('put')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="employee_id" class="form-control" id="employee_id">
                                        @foreach($employees as $employee)
                                            <option @selected($employee->id == $model->employee_id) value="{{$employee->id}}">{{$employee->full_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('employee_id')
                                    <span class="text-danger"> {{$message}}</span>
